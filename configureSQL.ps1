...
$akv_Content = (Invoke-WebRequest -Uri "https://$keyVaultName.vault.azure.net/secrets/AzDevOpsSqlPass?api-version=2016-10-01" -UseBasicParsing -Method GET -Headers @{Authorization="Bearer $KeyVaultToken"}).content
$value = ($akv_Content | ConvertFrom-JSON).value
$Password = $value

$targetsrvfull = $targetsrv + ".database.windows.net"
